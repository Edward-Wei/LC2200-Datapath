...
seqrom.txt, or converted assembly program), hit "ctrl+a" to highlight all the text and "ctrl+v" to copy 
...
=> MICOCompiler ROM Translator

The MICOCompiler ROM translator accepts as input a file specifying the various states
...
LogicWorks as a hex input file to a PROM or RAM module.  Note that everything
in the MICOCompiler ROM Translator is cAsE sEnSiTiVe.  If you're not familiar with
...
description of the grammar to use for your FSM input and some examples:
	...
	The MICOCompiler takes in an XML file that contains a list of all the states, 
